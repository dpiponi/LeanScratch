...
theorem descend_and_left {a b c : Prop} : (a → b) → a ∧ c → b ∧ c :=
begin
  intro h,
  intro ac,
  apply and.intro,
...
theorem descend_or_left {a b c : Prop} : (a → b) → a ∨ c → b ∨ c :=
assume h,
  assume ac,
    or.elim ac
...
theorem descend_imp_left {a b c : Prop} : (a → b) → (b → c) → (a → c) :=
 assume f, assume g, g ∘ f

theorem descend_imp_right {a b c : Prop} : (a → b) → (c → a) → (c → b) :=
 assume f, assume g, f ∘ g
 --function.comp

theorem descend_or_right {a b c : Prop} : (a → b) → c ∨ a → c ∨ b :=
assume h,
  assume ac,
    or.elim ac
      (assume hc, or.inl hc)
...
theorem descend_and_right {a b c : Prop} : (a → b) → c ∧ a → c ∧ b :=
begin
  intro h,
  intro ca,
  apply and.intro,
...
theorem descend_not {a b c : Prop} : (a → b) → ¬ b → ¬ a :=
begin
  intro h,
  intro nb,
  intro a,
...
with dobinary : expr → expr → expr → (name) → tactic name | u l e thm :=
  unify u l >> (do
    applyc thm,
    swap,
    exact e,
...
with dounary : expr → (name) → tactic name | e thm :=
  do
    applyc thm,
    swap,
    exact e,
